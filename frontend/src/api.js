...
// Base URL of your deployed Django backend
const API_URL = "https://career-path-ai-sc0h.onrender.com/api/"; // <--- make sure it ends with /api/
...
export const analyzeCareer = async (resumeText) => {
  if (!resumeText) throw new Error("Resume text is empty");
  const res = await api.post("career/analyzer/", { resume_text: resumeText });
  return res.data;
};
